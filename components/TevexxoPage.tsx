...
import { courses, features, navItems, projects, testimonials, type Course } from '@/lib/tevexxo-data';
...
function SearchOverlay({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const go = (type: 'Course' | 'Project') => {
    onClose();
    router.push(type === 'Course' ? '/courses' : '/projects');
  };
...
function Navbar({ onSearch, onDemo }: { onSearch: () => void; onDemo: () => void }) {
  const [menuOpen, setMenuOpen] = useState(false);
  return (
    <header className="absolute inset-x-0 top-0 z-40">
      <div className="container flex h-20 items-center justify-between">
        <Link href="/" className="inline-flex items-center gap-2" aria-label="Tevexxo home">
          <Logo className="h-9 w-9" priority />
          <span className="text-lg font-extrabold tracking-tight text-white">Tevexxo</span>
        </Link>
        <nav className="hidden items-center gap-6 lg:flex">
          {navItems.map((item, index) => (
            <Link key={item.label} href={item.href} className={`nav-link ${index === 0 ? 'active' : ''}`}>
              {item.label}
            </Link>
          ))}
        </nav>
      ...
      <AnimatePresence>
        {menuOpen && (
          ...
          >
            {navItems.map((item) => (
              <Link key={item.label} href={item.href} onClick={() => setMenuOpen(false)} className="block rounded-xl px-4 py-3 text-sm font-semibold text-white hover:bg-white/10">
                {item.label}
              </Link>
            ))}
            <div className="mt-2 flex items-center justify-between gap-2 px-1">
              <button onClick={() => { setMenuOpen(false); onSearch(); }} className="icon-button" aria-label="Search">
                <Search size={18} />
              </button>
              <button onClick={() => { setMenuOpen(false); onDemo(); }} className="orange-button flex-1">
      ...
      <div className="container relative z-10 grid min-h-[650px] items-center gap-10 pb-28 pt-28 lg:grid-cols-[.95fr_1.05fr] lg:pb-32 lg:pt-24">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
          ...
          <div className="mt-8 flex flex-wrap gap-3">
            <Link href="#courses" className="orange-button">
  ...
  return (
    ...
    >
      ...
      <div className="mt-3 flex items-center justify-between">
        ...
        <Link href="/courses" className={`flex h-8 w-8 items-center justify-center rounded-full border ${course.featured ? 'border-orange-500 bg-orange-500 text-white' : 'border-slate-200 text-slate-600'}`} aria-label={`Explore ${course.title}`}>
          <ArrowRight size={15} />
        </Link>
      </div>
    </motion.article>
    ...
    <section id="courses" className="section-space bg-slate-50">
      <div className="container">
        <SectionHeading
          ...
          title="Explore Our"
          accent="Top Courses"
          action={<Link href="/courses" className="hidden items-center gap-2 text-xs font-bold text-slate-600 sm:flex">View All Courses <ArrowRight size={14} /></Link>}
        />
        ...
        <Link href="/courses" className="mt-6 flex items-center justify-center gap-2 text-xs font-bold text-orange-500 sm:hidden">
    ...
    <section id="projects" className="section-space bg-white">
      <div className="container">
        <SectionHeading
          ...
          accent="Get Hired."
          action={<Link href="/projects" className="hidden items-center gap-2 text-xs font-bold text-slate-600 sm:flex">View All Projects <ArrowRight size={14} /></Link>}
        />
            ...
            >
              ...
              <div className="p-5">
                ...
                <div className="mt-5 flex items-center justify-between border-t border-slate-100 pt-4 text-[10px] font-semibold">
                  ...
                  <Link href={`/projects/${project.slug}`} className="flex items-center text-orange-500">
        ...
        {footerLinks.map((col) => (
          <div key={col.title}>
            ...
            <ul className="mt-4 space-y-2 text-xs">
              {col.links.map((link) => (
                <li key={link.label}>
                  <Link href={link.href} className="hover:text-orange-400">{link.label}</Link>
                </li>
              ))}
            </ul>
          </div>
